...
<%  HcComplainService hcComplainSvc = new HcComplainService();
	List<HcComplainVO> list = hcComplainSvc.getAll();
...
<b>Hello! <%= employeeVO.getEmpId() %> </b>
        ...
        <!-- 長照申訴 ========================================================================== -->
           ...
           <table class="table table-striped">
                ...
                <c:forEach var="hcComplainVO" items="${list}" begin="<%=pageIndex%>" end="<%=pageIndex+rowsPerPage-1%>">
                    <!--將修改的那一筆加入對比色而已-->
                    <tr class="row-content"  ${(hcComplainVO.complainNo == param.complainNo) ? 'text-primary':''}" valign='middle'>
                        <td>
                        <FORM METHOD="post" ACTION="<%=request.getContextPath()%>/hccomplain/hccomplain.do" style="margin-bottom: 0px;">
						    <button type="submit">${hcComplainVO.complainNo}</button>
                        ...
                        <td>${sdf.format(hcComplainVO.detailDate)}</td>
                        <td>${hcComplainVO.complainReply}</td>
                        <td>
                        <c:choose>
						    <c:when test="${hcComplainVO.complainStatus == '未處理'}">
						       <span class="fa edit label label-danger" style="height: 30px;">${hcComplainVO.complainStatus}</span>
						    </c:when>
						    <c:otherwise>
						       <span class="fa edit label label-default" style="height: 30px;">${hcComplainVO.complainStatus}</span>
						    </c:otherwise>
						</c:choose>
                        </td>
                        <td>
