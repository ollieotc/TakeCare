...
<%  
	...
	List<MealComplainVO> list = mealComplainSvc.getAll();
        ...
        <!-- 送餐申訴 ========================================================================== -->
           ...
           <table class="table table-striped">
                ...
                <c:forEach var="mealComplainVO" items="${list}" begin="<%=pageIndex%>" end="<%=pageIndex+rowsPerPage-1%>">
                   <!--將修改的那一筆加入對比色而已-->
                    <tr class="row-content ${(mealComplainVO.complainNo == param.complainNo) ? 'text-primary':''}" valign='middle'>
                        <td >
                        <FORM METHOD="post" ACTION="<%=request.getContextPath()%>/mealcomplain/mealcomplain.do" style="margin-bottom: 0px;">
						    <button type="submit">${mealComplainVO.complainNo}</button>
                        ...
                        <td>${sdf.format(mealComplainVO.detailDate)}</td>
                        <td>${mealComplainVO.complainReply}</td>
                        <td>
                       	<c:choose>
						    <c:when test="${mealComplainVO.complainStatus == '未處理'}">
						       <span class="fa edit label label-danger" style="height: 30px;">${mealComplainVO.complainStatus}</span>
						    </c:when>
						    <c:otherwise>
						       <span class="fa edit label label-default" style="height: 30px;">${mealComplainVO.complainStatus}</span>
						    </c:otherwise>
						</c:choose>
